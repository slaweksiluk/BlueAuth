...
int get_user_config(const char **user, char* btaddr, int *port, char *mobile_id,
    ...
    // Sprawdz czy poliki konfiguracyjne istnieja
    if(!(file_exists(inifile_users) && file_exists(inifile_mobiles)))
        return E_INI_FILES_NOT_EXIST;
...
//pobierz nazwe telfonu przypisanego do uzytkownika
                ini_gets(section, "mobile", "0", mobile, sizearray(mobile), inifile_users);
//  pobierz wartosc progu RSSI uzytkownika
                min_rssi = ini_getf(section, "rssi", -5.0, inifile_users);
